...
QSAM='QSAM1' ##<< QSAM matrix 4*n, where n is a number of properties; possible values are 'QSAM1' and 'QSAM2'
  ...
  if (!class(QSAM)%in%c('character', 'data.frame', 'matrix')){
      stop('Wrong argument: correct the argument QSAM! Choose one of predefined matrices ("QSAM1", "QSAM2") or set it as a data.frame or a matrix.\n')
  }
  ...
  if(class(QSAM)%in%c('data.frame', 'matrix')){
    ...
    if (sort(rownames(QSAM)%in%c('A', 'C', 'G', 'T', 'U', 'a', 'c', 'g', 't', 'u'))[1]==FALSE){
      warning('Chech rownames in the QSAM matrix!')
    }
  }
 
  
  if (is(QSAM)[1]=='character'){
    if (!QSAM%in%c('QSAM1', 'QSAM2')){
      stop('Wrong argument: correct the argument QSAM! Choose one of predefined matrices ("QSAM1", "QSAM2") or set it as a data.frame or a matrix.\n')
    }
